...
// const preoloadedThemes = await Promise.all(
//   shikiThemes.map(async (theme) => {
//     const loadedTheme = await theme.import();
...
//     return {
//       id: theme.name,
//       colors: getThemePrimaryColors(theme),
//     };
//   });
// });

// export function getThemePrimaryColors(theme?: RawTheme | BundledThemeInfo) {
//   const settings =
//     theme && "settings" in theme
//       ? theme.settings
//       : theme && "tokenColors" in theme
//         ? theme.tokenColors
//         : [];
//   if (settings.length === 0)
//     return preoloadedThemes.find((t) => t.id === theme.id)?.colors;
//   const colors = settings.reduce<{ key: string; color?: string }[]>(
//     (acc, item) => {
//       const scope = Array.isArray(item.scope) ? item.scope : [item.scope];
...
//       const key = scope[index] as string;
