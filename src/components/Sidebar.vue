...
<template>
  <OnClickOutside v-if="store.showUI" @trigger="isExpanded = false">
    <aside>
      ...
      <div
        class="fixed inset-x-0 bottom-[56px] z-10 max-h-[calc(30svh-48px)] content-start overflow-auto border-t border-zinc-800 bg-zinc-900 transition-[height] sm:static sm:grid sm:max-h-screen sm:w-[282px] sm:overflow-auto sm:border-r sm:border-t-0 sm:transition-none pwa:sm:border-t pwa:sm:border-t-black pwa:sm:shadow-[inset_0_1px_0_rgb(39_39_42)]"
        :class="{
          hidden: !isExpanded,
        }"
      >
